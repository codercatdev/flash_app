...
curl -X POST http://localhost:8888/flash_app_gpu_worker/runsync \
  -H "Content-Type: application/json" \
  -d '{"input": {"input_data": {"message": "Hello GPU!"}}}'

# Queue-based CPU worker
curl -X POST http://localhost:8888/flash_app_cpu_worker/runsync \
  -H "Content-Type: application/json" \
  -d '{"input": {"input_data": {"message": "Hello CPU!"}}}'

# Load-balanced HTTP endpoint
curl -X POST http://localhost:8888/flash_app_lb_worker/process \
  -H "Content-Type: application/json" \
  -d '{"input_data": {"message": "Hello from LB!"}}'

# Load-balanced health check
curl http://localhost:8888/flash_app_lb_worker/health
```
...
@Endpoint(name="flash_app_gpu_worker", gpu=GpuType.ANY, dependencies=["torch"])
...
@Endpoint(name="flash_app_cpu_worker", cpu="cpu3c-1-2")
...
api = Endpoint(name="flash_app_lb_worker", cpu="cpu3c-1-2", workers=(1, 3))
